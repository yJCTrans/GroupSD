...
def create_model(args):
    model = models.create(args.arch, norm=True, BNNeck=args.BNNeck, SD=args.SD, conv_stem=True, DSU_stem=False)
...
def main_worker(args):
    ...
    if args.multi_source:
        ...
        dataset_src = get_data(args.dataset_src1, args.data_dir, combine_all=True)
    ...
    trainer = Trainer(args, model, memories, criterion, num_classes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=64)    # 32
    parser.add_argument('--test-batch-size', type=int, default=128)    # 256
