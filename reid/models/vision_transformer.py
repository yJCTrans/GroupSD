...
class SEModule(nn.Module):
    def __init__(self, channels, reduction=16):
        ...
        self.register_buffer('last_weight', None)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            current_mean = y.mean(dim=0)
            current_global_mean = current_mean.mean()
            if self.weight_sum is None:
                self.weight_sum = current_mean
            else:
                self.weight_sum += current_mean
            self.weight_count += 1
            self.last_weight = current_mean

        weighted_x = x * y
        return weighted_x, current_global_mean
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, conv_stem=False, DSU_stem=False):
        super().__init__()
        ...
        self.DSU = DSU(p=0.5)
        self.DSU_stem = DSU_stem

        self.se1 = SEModule(channels=embed_dim)
        self.se2 = SEModule(channels=embed_dim)
        self.se3 = SEModule(channels=embed_dim)
        self.se4 = SEModule(channels=embed_dim)
    ...
    def forward(self, x):
        ...
        x1 = list_out[-1]
        x = [self.fc(x) for x in list_out]
        block_number = random.randint(0, len(self.blocks) - 1)
        x_random_block = x[block_number]

        low_f_0, wei_f_0 = self.se1(x[0])
        low_f_1, wei_f_1 = self.se1(x[1])
        low_f_2, wei_f_2 = self.se1(x[2])

        low_f_3, wei_f_3 = self.se2(x[3])
        low_f_4, wei_f_4 = self.se2(x[4])
        low_f_5, wei_f_5 = self.se2(x[5])

        high_f_6, wei_f_6 = self.se3(x[6])
        high_f_7, wei_f_7 = self.se3(x[7])
        high_f_8, wei_f_8 = self.se3(x[8])

        high_f_9, wei_f_9 = self.se4(x[9])
        high_f_10, wei_f_10 = self.se4(x[10])
        high_f_11, wei_f_11 = self.se4(x[11])

        low_1 = low_f_0 + low_f_1 + low_f_2

        mid_1 = low_f_3 + low_f_4 + low_f_5

        mid_2 = high_f_6 + high_f_7 + high_f_8

        high_1 = high_f_9 + high_f_10 + high_f_11

        out_list = [low_f_0, low_f_1, low_f_2, low_f_3, low_f_4, low_f_5, high_f_6, high_f_7, high_f_8, high_f_9, high_f_10, high_f_11]
        out = [low_1,  mid_1, mid_2, high_1]
        return x1, x_random_block, out, out_list
