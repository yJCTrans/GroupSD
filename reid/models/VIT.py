...
class build_transformer(nn.Module):
    def __init__(self, num_features=0, norm=True, BNNeck=True, SD=False, conv_stem=True, DSU_stem=False):
        ...
        self.base = TransReID(
            ...
            drop_rate=0.0, attn_drop_rate=0.0, conv_stem=conv_stem, DSU_stem=DSU_stem)
    ...
    def forward(self, x):
        ...
        x, x_rb, out, out_list = self.base(x)
        ...
        if self.SD:
            if self.BNNeck:
                return bn_x, bn_x_rb, tri_features, rb_features, out, out_list
            else:
                return bn_x, bn_x_rb, out, out_list
        else:
            if self.BNNeck:
